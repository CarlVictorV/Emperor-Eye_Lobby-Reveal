                    ...
                    while 1:
                        ...
                        for i in range(len(chat)):
                            if chat[i]['type'] == "championSelect":
                                ...
                                request = "/lol-chat/v1/conversations/" + \
                                    str(lobbyID) + "/messages"
                                sleep(1)
                                ...
                                if len(nameArr) == nbPlayer:
                                    ...
                                    for i in range(len(nameArr)):
                                        try:
                                            usera = watcher.summoner.by_name(
                                                my_region, nameArr[i])
                                            # sleep(0.3)
                                            user_id = usera['id']
                                            ...
                                            try:
                                                ranked_stats = watcher.league.by_summoner(
                                                    my_region, user_id)
                                                ...
                                                print(
                                                    "error in get ranked stats")
                                                await connection.request('post', request, headers=headers, data={"type": "chat", "body": "error getting " + str(nameArr[i]) + " ranked datas :'("})
                                        except ApiError:
                                            print(
                                                "error in get user id. check your api key ?")
                                            await connection.request('post', request, headers=headers, data={"type": "chat", "body": "error getting " + str(nameArr[i]) + " user id :'("})
                                    await connection.request('post', request, headers=headers, data={"type": "chat", "body": "https://ph.op.gg/multi/query=" + nospaces[0] + "%2C" + nospaces[1] + "%2C" + nospaces[2] + "%2C" + nospaces[3] + "%2C" + nospaces[4]})
                                    showNotInChampSelect = True
                                    checkForLobby = True
